...
describe('router gateway routes', () => {
  ...
  test('rejects oversized public request bodies before routing upstream', async () => {
    ...
    const response = await app.request('/quote', {
      ...
      body: 'x'.repeat(MAX_GATEWAY_JSON_BODY_BYTES + 1)
    })
    const body = await response.json()
...
function internalLimitOrder() {
  return {
    ...
    funding_vault: {
      vault: {
        deposit_vault_address: '0x3333333333333333333333333333333333333333'
      }
    },
    cancellation_secret:
