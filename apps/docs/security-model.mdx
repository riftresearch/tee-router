...
The security model of multichain smart contracts is fundamentally different from a smart contract on a single chain like Ethereum, although the goal is the same: programmatic, transparent execution that cannot be altered (e.g. stealing or selectively freezing user funds) by an operator.
