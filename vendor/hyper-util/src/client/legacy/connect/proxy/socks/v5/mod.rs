...
impl SocksConfig {
    ...
    where
        T: Read + Write + Unpin,
    {
        // RIFT PATCH (the only local change in this vendored crate): `http::Uri::host()`
        // returns IPv6 literals WITH brackets ("[2606:..]"), which fails the IpAddr
        // parse below and gets misclassified as Address::Domain — the SOCKS proxy is
        // then asked to DNS-resolve a bracketed literal and replies host-unreachable.
        // reqwest's socks local-resolve path hands us exactly such hosts (it must
        // bracket resolved IPv6 addresses to build a valid Uri). A bracketed string
        // is never a valid domain name, so unwrapping here cannot break domains.
