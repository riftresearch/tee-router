...
impl<S: Sleeper> AsyncClient<S> {
    ...
    pub fn from_builder(builder: Builder) -> Result<Self, Error> {
        ...
        if let Some(proxy) = &builder.proxy {
            client_builder = client_builder.proxy(reqwest::Proxy::all(proxy).map_err(|e| {
                Error::ReqwestWithContext {
                    error: e,
                    context: "Failed to create proxy".to_string(),
                }
            })?);
        }
        ...
        Ok(AsyncClient {
            ...
            client: client_builder
                ...
                .map_err(|e| Error::ReqwestWithContext {
                    error: e,
                    context: "Failed to build client".to_string(),
                })?,
        ...
        if !response.status().is_success() {
            return Err(Error::HttpResponse {
                ...
                message: response
                    ...
                    .map_err(|e| Error::ReqwestWithContext {
                        error: e,
                        context: "Failed to get response text".to_string(),
                    })?,
            });
        }

        Ok(deserialize::<T>(&response.bytes().await.map_err(|e| {
            Error::ReqwestWithContext {
                error: e,
                context: "Failed to get response bytes".to_string(),
            }
        })?)?)
    }
    ...
    async fn get_response_json<T: serde::de::DeserializeOwned>(
        ...
        response
            ...
            .map_err(|e| Error::ReqwestWithContext {
                error: e,
                context: "Failed to get response json".to_string(),
            })
    }
        ...
        if !response.status().is_success() {
            return Err(Error::HttpResponse {
                ...
                message: response
                    ...
                    .map_err(|e| Error::ReqwestWithContext {
                        error: e,
                        context: "get_response_hex: Failed to get response text".to_string(),
                    })?,
            });
        }
    ...
    async fn post_request_hex<T: Encodable>(&self, path: &str, body: T) -> Result<(), Error> {
        ...
        let response = self.client.post(url).body(body).send().await.map_err(|e| {
            Error::ReqwestWithContext {
                error: e,
                context: "post_request_hex: Failed to send request".to_string(),
            }
        })?;
        ...
        if !response.status().is_success() {
            return Err(Error::HttpResponse {
                ...
                message: response
                    ...
                    .map_err(|e| Error::ReqwestWithContext {
                        error: e,
                        context: "post_request_hex: Failed to get response text".to_string(),
                    })?,
            });
        }
    ...
    /// Sends a GET request to the given `url`, retrying failed attempts for
    /// retryable HTTP responses and transient request errors until max retries
    /// hit.
    async fn get_with_retry(&self, url: &str) -> Result<Response, Error> {
        ...
        loop {
            match self.client.get(url).send().await {
                ...
                Err(err) if attempts < self.max_retries && is_request_error_retryable(&err) => {
                    S::sleep(delay).await;
                    attempts += 1;
                    delay *= 2;
                }
                Err(err) => {
                    return Err(Error::ReqwestWithContext {
                        error: err,
                        context: format!(
                            "Failed to send request after {attempts} attempts and {delay:?} delay"
                        ),
                    })
                }
            }
...
/// Determines if a reqwest error represents a transient network issue that should be retried.
fn is_request_error_retryable(err: &reqwest::Error) -> bool {
    // A TCP connect error means the process/socket is unavailable. Retrying it
    // with exponential backoff inside every order task makes a dead Esplora
    // instance look like router latency, so fail fast and let callers surface
    // the upstream health issue.
    if err.is_connect() {
        return false;
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn connection_refused_fails_without_retry_backoff() {
        let listener = TcpListener::bind("127.0.0.1:0").expect("bind local test port");
        let port = listener.local_addr().expect("local addr").port();
        drop(listener);

        let client = AsyncClient::<DefaultSleeper>::from_builder(Builder::new(&format!(
            "http://127.0.0.1:{port}"
        )))
        .expect("client");
        ...
        let err = client
            .get_fee_estimates()
            .await
            .expect_err("closed local port should refuse connections");

        assert!(
            started.elapsed() < Duration::from_secs(1),
            "connection refused should not consume retry backoff: {err:?}"
        );
        if let Error::ReqwestWithContext { context, .. } = err {
            assert!(
                !context.contains("after 7 attempts"),
                "connection refused was retried: {context}"
            );
        }
    }
}
