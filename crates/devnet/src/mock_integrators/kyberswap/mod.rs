...
mod tests {
    ...
    async fn spawn_mock_kyberswap_server_with(config: MockIntegratorConfig) -> MockIntegratorServer {
    ...
    fn routes_url(server: &MockIntegratorServer, token_in: &str, token_out: &str, amount_in: &str) -> String {
        format!(
    ...
    async fn routes_sell_eth_to_usdc_uses_prices_and_decimals() {
        ...
        let url = routes_url(&server, KYBERSWAP_NATIVE_TOKEN, ETH_USDC, "1000000000000000000");
    ...
    async fn routes_sell_usdc_to_eth_uses_prices_and_decimals() {
        ...
        assert_eq!(body["data"]["routeSummary"]["amountOut"], "20000000000000000");
    }
    ...
    async fn post_build(server: &MockIntegratorServer, body: Value) -> (reqwest::StatusCode, Value) {
        let response = reqwest::Client::new()
            .post(format!("{}/ethereum/api/v1/route/build", server.kyberswap_url()))
            .json(&body)
            .send()
            .await
            .expect("http post");
    ...
    async fn route_build_encodes_swap_calldata_and_zero_value_for_erc20_input() {
        ...
        assert_eq!(decoded.execution.desc.dstReceiver, Address::from_str(RECIPIENT).unwrap());
        assert_eq!(decoded.execution.desc.amount, U256::from(60000000u64));
        assert_eq!(decoded.execution.desc.minReturnAmount, U256::from(20000000000000000u64));
    }
    ...
    async fn route_build_stale_quote_injection_returns_4008() {
        let server = spawn_mock_kyberswap_server().await;
        server.set_kyberswap_transaction_stale_quote_fail_next_n(1).await;
        let (status, body) = post_build(
            &server,
            json!({
                "routeSummary": route_summary(ETH_USDC, KYBERSWAP_NATIVE_TOKEN, "60000000", "20000000000000000"),
