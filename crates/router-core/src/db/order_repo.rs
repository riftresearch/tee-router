...
#[derive(Debug, Clone)]
pub struct HyperliquidSpotRefundAttemptPlan {
    pub source_custody_vault_id: Uuid,
    pub legs: Vec<OrderExecutionLeg>,
    pub steps: Vec<OrderExecutionStep>,
    pub failure_reason: serde_json::Value,
...
impl OrderRepository {
    ...
    pub async fn create_refund_attempt_from_hyperliquid_spot(
        &self,
        order_id: Uuid,
        failed_attempt_id: Uuid,
        plan: HyperliquidSpotRefundAttemptPlan,
        now: DateTime<Utc>,
    ) -> RouterCoreResult<ExecutionAttemptMaterializationRecord> {
        let started = Instant::now();
        let result = async {
            if plan.legs.is_empty() || plan.steps.is_empty() {
                return Err(RouterCoreError::InvalidData {
                    message: "HyperliquidSpot refund plan must contain legs and steps".to_string(),
                });
            }
        ...
        .await;
        telemetry::record_db_query(
            "order.create_refund_attempt_from_hyperliquid_spot",
            result.is_ok(),
            started.elapsed(),
        );
        result
