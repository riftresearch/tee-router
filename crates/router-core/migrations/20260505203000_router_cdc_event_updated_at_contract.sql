...
BEGIN
    IF TG_OP = 'DELETE' THEN
        CASE TG_TABLE_NAME
            ...
            WHEN 'deposit_vaults' THEN
                ...
                event_updated_at := OLD.updated_at;
                SELECT cv.order_id INTO event_order_id FROM public.custody_vaults cv WHERE cv.id = OLD.id;
    ...
    ELSE
        CASE TG_TABLE_NAME
            ...
            WHEN 'deposit_vaults' THEN
                ...
                event_updated_at := NEW.updated_at;
                SELECT cv.order_id INTO event_order_id FROM public.custody_vaults cv WHERE cv.id = NEW.id;
            WHEN 'custody_vaults' THEN
                record_id := NEW.id;
                event_order_id := NEW.order_id;
                event_watch_id := NEW.id;
    ...
    IF TG_TABLE_NAME IN ('market_order_quotes', 'limit_order_quotes') AND event_order_id IS NULL THEN
        IF TG_OP = 'DELETE' THEN
            RETURN OLD;
        END IF;
        RETURN NEW;
    ...
    event_updated_at := COALESCE(event_updated_at, event_order_updated_at, now());
    ...
    PERFORM pg_catalog.pg_logical_emit_message(true, 'rift.router.change', payload::text);
