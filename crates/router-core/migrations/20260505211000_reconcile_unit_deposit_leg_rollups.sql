...
SET
    ...
    actual_amount_in = CASE
        WHEN rolled_up.completed THEN COALESCE(
            CASE
                WHEN (rolled_up.first_response_json #>> '{observed_state,provider_observed_state,source_amount}') ~ '^[0-9]+$'
                    THEN rolled_up.first_response_json #>> '{observed_state,provider_observed_state,source_amount}'
                WHEN (rolled_up.first_response_json #>> '{observed_state,provider_observed_state,sourceAmount}') ~ '^[0-9]+$'
                    THEN rolled_up.first_response_json #>> '{observed_state,provider_observed_state,sourceAmount}'
                WHEN (rolled_up.first_response_json->>'amount') ~ '^[0-9]+$'
    ...
    actual_amount_out = CASE
        WHEN rolled_up.completed THEN COALESCE(
            ...
            CASE
                WHEN (rolled_up.last_response_json #>> '{observed_state,provider_observed_state,destination_amount}') ~ '^[0-9]+$'
                    THEN rolled_up.last_response_json #>> '{observed_state,provider_observed_state,destination_amount}'
                WHEN (rolled_up.last_response_json #>> '{observed_state,provider_observed_state,destinationAmount}') ~ '^[0-9]+$'
                    THEN rolled_up.last_response_json #>> '{observed_state,provider_observed_state,destinationAmount}'
                WHEN (rolled_up.last_response_json #>> '{observed_state,provider_observed_state,source_amount}') ~ '^[0-9]+$'
                    THEN rolled_up.last_response_json #>> '{observed_state,provider_observed_state,source_amount}'
                WHEN (rolled_up.last_response_json #>> '{observed_state,provider_observed_state,sourceAmount}') ~ '^[0-9]+$'
                    THEN rolled_up.last_response_json #>> '{observed_state,provider_observed_state,sourceAmount}'
                WHEN (rolled_up.last_response_json->>'amount') ~ '^[0-9]+$'
