...
| `hyperliquid_spot` | Aligned as a monochain venue | Provider id, monochain fixed-pair taxonomy, static spot assets, `HyperliquidTrade` transition, `ExchangeProvider` adapter, startup network config, route costs, route minimums, planner/executor/refund support, timeout/cancel recovery, Hyperliquid custody calls, mock `/info` + `/exchange`, local integration tests, and live Hyperliquid differential tests are present. |
