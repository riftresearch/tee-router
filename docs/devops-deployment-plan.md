...
1. Provider/chain-specific override, e.g. `CCTP_PROXY_URL`,
   `KYBERSWAP_PROXY_URL`, `BASE_RPC_PROXY_URL`, or `COINBASE_PROXY_URL`.
...
- `UPSTREAM_PROXY_URL` general SOCKS5 fallback, `HYPERUNIT_PROXY_URL` dedicated
  ...
  `KYBERSWAP_PROXY_URL`, `RELAY_PROXY_URL`,
  `NEAR_INTENTS_PROXY_URL`, `MAYAN_PROXY_URL`, `CHAINFLIP_PROXY_URL`,
  `GARDEN_PROXY_URL`, `CHAINALYSIS_PROXY_URL`, `COINBASE_PROXY_URL`
- `HYPERUNIT_API_URL`
...
- `KYBERSWAP_API_URL`; KyberSwap has no API key, and `x-client-id` is
  ...
  public 3 rps limit and use `KYBERSWAP_PROXY_URL` only for egress selection.
