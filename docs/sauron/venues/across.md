...
|---|---|---|---|
| Origin deposit | Source chain | `FundsDeposited(depositId, ...)` from SpokePool | Confirms our deposit is locked in the bridge |
| Destination fill | Dest chain | `FilledRelay(depositId, ...)` from SpokePool | Confirms a relayer has filled the recipient — bridge complete |
...
- `eth_subscribe('logs', { address: SpokePool_origin, topics: [FundsDeposited.topic0] })` on the source chain.
- `eth_subscribe('logs', { address: SpokePool_dest, topics: [FilledRelay.topic0] })` on the destination chain.
...
| Mechanism | Use case |
|---|---|
| `eth_getLogs` for `FundsDeposited` and `FilledRelay` | Backfill on WS reconnect or cold start |
...
2. **Emit chain hints with venue context**: when matching `FundsDeposited` or `FilledRelay`, tag the emitted `DetectorHint` with venue-specific kinds (`AcrossOriginDeposited`, `AcrossDestinationFilled`).
...
2. **Origin observation**: chain backend WS picks up `FundsDeposited(depositId=ours)` on origin chain → emit `DetectorHint { hint_kind: AcrossOriginDeposited, depositId, tx_hash }`.
3. **Destination observation**: chain backend WS picks up `FilledRelay(depositId=ours)` on destination chain → emit `DetectorHint { hint_kind: AcrossDestinationFilled, depositId, fill_tx_hash, output_amount }`.
...
| Destination fill amount differs from expected (relayer fee changed) | The fill amount in `FilledRelay` is what we got. If less than `min_amount_out`, T-router treats as a partial-success and may need to top up downstream legs |
