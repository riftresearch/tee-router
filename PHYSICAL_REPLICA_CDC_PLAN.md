...
Replace the writable logical subscriber replica with a physical Postgres standby
on Railway so the replica derives schema and data directly from the Phala primary
WAL stream. Use logical decoding on the physical standby for event delivery to
Sauron and the future admin dashboard.
...
- Native logical subscriber drift cannot wedge replication when primary schema
  changes.
...
- Logical decoding on a standby requires Postgres 16+ and careful WAL/slot lag
  monitoring.
...
- `router-postgres-primary`
  - Runs in Phala.
  - Postgres 18.
  - `wal_level = logical` so the physical standby can support logical decoding.
...
- `sauron-worker-v3`
  ...
  - Consumes a logical decoding stream from the physical standby.
...
2. Prove local primary/physical-standby/logical-decoding-on-standby behavior.
3. Deploy a new Railway Postgres 18 physical standby alongside the current
   logical subscriber.
...
8. Retire the old writable logical subscriber once Sauron and admin reads are
   stable on the physical standby.
...
- `railway/router-physical-standby/`
  - Postgres 18 image that waits for the Phala primary, creates/uses a physical
    ...
    standby with logical decoding enabled.
...
- `bin/sauron/src/cdc.rs`
  - Logical-decoding consumer support using a standby-side slot and
    `test_decoding` output.
- `bin/sauron/src/runtime.rs`
  - Split router read pool from Sauron state pool.
  - Added selectable event sources:
    - `notify` for legacy local/logical-replica tests.
    - `cdc` for physical-standby production.
    - `reconcile` for emergency reconcile-only mode.
...
1. Started a Postgres 18 primary with `wal_level=logical`.
2. Started `tee-router-physical-standby:local`.
3. Confirmed the standby reported `pg_is_in_recovery() = true`.
4. Created a `test_decoding` logical slot on the standby.
...
- Standby logical decoding slot exists and CDC lag is below alert threshold.
