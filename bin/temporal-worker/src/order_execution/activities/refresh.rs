...
async fn boundary_best_provider_quote(
    ...
    router_core::services::route_costs::rank_transition_paths_structurally(&mut paths);
...
impl QuoteRefreshActivities {
    ...
    #[activity]
    pub async fn compose_leg_boundary_requote_attempt(
        self: Arc<Self>,
        _ctx: ActivityContext,
        input: ComposeLegBoundaryRequoteInput,
    ) -> Result<LegBoundaryRequoteAttemptShape, ActivityError> {
        record_activity("compose_leg_boundary_requote_attempt", async move {
            let deps = self.deps()?;
            let order = deps
                .db
                .orders()
                ...
                .await
                .map_err(OrderActivityError::db_query)?;
            let source_attempt = deps
                .db
                .orders()
                .get_execution_attempt(input.source_attempt_id.inner())
                .await
                .map_err(OrderActivityError::db_query)?;
            if source_attempt.order_id != input.order_id.inner() {
                return Err(invariant_error(
                    "boundary_requote_attempt_order",
                    format!(
                        "attempt {} does not belong to order {}",
                        source_attempt.id,
                        input.order_id.inner()
            ...
            if source_attempt.status != OrderExecutionAttemptStatus::Active {
                return Ok(leg_boundary_requote_not_needed(
                    input.order_id.inner(),
                    input.source_attempt_id.inner(),
                    input.completed_step_id.inner(),
                    json!({
                        "reason": "source_attempt_not_active",
                        "source_attempt_status": source_attempt.status.to_db_string(),
                    }),
                ));
            }

            let completed_step = deps
                .db
                .orders()
                .get_execution_step(input.completed_step_id.inner())
                .await
                .map_err(OrderActivityError::db_query)?;
            if completed_step.order_id != input.order_id.inner()
                || completed_step.execution_attempt_id != Some(source_attempt.id)
            {
                return Err(invariant_error(
                    "boundary_requote_completed_step_owner",
                    format!(
                        "step {} does not belong to order {} attempt {}",
                        completed_step.id,
                        input.order_id.inner(),
            ...
            if completed_step.status != OrderExecutionStepStatus::Completed {
                return Ok(leg_boundary_requote_not_needed(
                    ...
                    input.completed_step_id.inner(),
                    json!({
                        "reason": "completed_step_not_terminal",
                        "completed_step_status": completed_step.status.to_db_string(),
                    }),
                ));
            }
            let Some(completed_leg_id) = completed_step.execution_leg_id else {
                return Ok(leg_boundary_requote_not_needed(
                    input.order_id.inner(),
                    input.source_attempt_id.inner(),
                    input.completed_step_id.inner(),
                    json!({ "reason": "completed_step_has_no_execution_leg" }),
                ));
            };
            let Some(completed_leg) = deps
                .db
                .orders()
                .refresh_execution_leg_from_actions(completed_leg_id)
                .await
                .map_err(OrderActivityError::db_query)?
            else {
                return Ok(leg_boundary_requote_not_needed(
                    ...
                    input.source_attempt_id.inner(),
                    input.completed_step_id.inner(),
                    json!({ "reason": "execution_leg_not_found" }),
                ));
            };
            if completed_leg.execution_attempt_id != Some(source_attempt.id) {
                return Err(invariant_error(
                    "boundary_requote_completed_leg_owner",
                    format!(
            ...
            if completed_leg.status != OrderExecutionStepStatus::Completed {
                return Ok(leg_boundary_requote_not_needed(
                    ...
                    input.completed_step_id.inner(),
                    json!({
                        "reason": "execution_leg_not_completed",
                        "execution_leg_id": completed_leg.id,
                        "execution_leg_status": completed_leg.status.to_db_string(),
                    }),
                ));
            }
            if completed_leg.output_asset == order.destination_asset {
                return Ok(leg_boundary_requote_not_needed(
                    input.order_id.inner(),
                    input.source_attempt_id.inner(),
                    input.completed_step_id.inner(),
                    json!({
                        "reason": "completed_leg_reached_destination",
                        "execution_leg_id": completed_leg.id,
                    }),
                ));
            }
            if completed_leg.actual_amount_out.is_none() {
                return Ok(leg_boundary_requote_untenable(
                    input.order_id.inner(),
                    input.source_attempt_id.inner(),
                    input.completed_step_id.inner(),
                    format!(
            ...
            let source_vault_id = order.funding_vault_id.ok_or_else(|| {
                refresh_materialization_error(format!(
                    "order {} cannot re-quote without a funding vault",
                    order.id
                ))
            })?;
            let source_vault = deps
                ...
                .await
                .map_err(OrderActivityError::db_query)?;
            let source_attempt_steps = deps
                .db
                .orders()
                .get_execution_steps_for_attempt(source_attempt.id)
                .await
                .map_err(OrderActivityError::db_query)?;
            let completed_leg_steps = source_attempt_steps
                .iter()
                .filter(|step| step.execution_leg_id == Some(completed_leg.id))
                .cloned()
                .collect::<Vec<_>>();
            let (start_node, context) = boundary_requote_context(
                &deps,
                &order,
                &source_vault,
                &completed_leg,
            ...
            .await?;
            let now = Utc::now();
            let Some((path, provider_quote)) =
                boundary_best_provider_quote(&deps, &order, start_node, &context).await?
            else {
                return Ok(leg_boundary_requote_untenable(
                    input.order_id.inner(),
                    input.source_attempt_id.inner(),
                    input.completed_step_id.inner(),
                    format!(
                        "no executable route from completed leg {} at {} {} to {} {}",
                        completed_leg.id,
                        completed_leg.output_asset.chain,
                        completed_leg.output_asset.asset,
                        order.destination_asset.chain,
                        order.destination_asset.asset
                    ),
                ));
            };
            let requoted_quote = boundary_requoted_market_order_quote_exact_in(
                &order,
                context.source_asset.clone(),
                &path,
                provider_quote,
                now,
            );
            let mut plan = deps
                ...
                .plan_from_current_location(
                    ...
                    MarketOrderPlanCurrentLocationStart {
                        initial_source_role: context.initial_source_role,
                        initial_hyperliquid_custody: context.initial_hyperliquid_custody.clone(),
                        step_index: completed_step.step_index + 1,
                        leg_index: completed_leg.leg_index + 1,
                        planned_at: now,
                    },
                )
                .map_err(|source| refresh_materialization_error(source.to_string()))?;
            for leg in &mut plan.legs {
                set_json_value(
                    ...
                    json!(source_attempt.id),
                );
                set_json_value(
                    &mut leg.details,
                    "requoted_from_execution_leg_id",
                    json!(completed_leg.id),
                );
                set_json_value(
                    &mut leg.details,
                    "requoted_quote_id",
                    json!(requoted_quote.id),
                );
            }
            for step in &mut plan.steps {
                set_json_value(
            ...
            let reason = json!({
                ...
                "completed_leg_id": completed_leg.id,
                "current_asset": {
                    "chain": completed_leg.output_asset.chain.as_str(),
                    "asset": completed_leg.output_asset.asset.as_str(),
                },
                "available_amount": context.amount_in.clone(),
                "requoted_quote_id": requoted_quote.id,
                "path_id": plan.path_id,
            });
            let input_custody_snapshot = json!({
                "source_kind": "completed_leg_boundary",
                "source_attempt_id": source_attempt.id,
                "completed_step_id": completed_step.id,
                "completed_step_index": completed_step.step_index,
                "completed_step_type": completed_step.step_type.to_db_string(),
                "completed_leg_id": completed_leg.id,
                "completed_leg_index": completed_leg.leg_index,
                "amount_in": context.amount_in.clone(),
                "source_asset": {
                    "chain": completed_leg.output_asset.chain.as_str(),
                    "asset": completed_leg.output_asset.asset.as_str(),
                ...
                "source_custody_vault_role": context
                    .initial_source_role
                    .map(CustodyVaultRole::to_db_string),
                "hyperliquid_custody": context.initial_hyperliquid_custody.as_ref().map(|custody| json!({
                    "role": custody.role.to_db_string(),
                    "asset": custody.asset.as_ref().map(|asset| json!({
                        "chain": asset.chain.as_str(),
                        "asset": asset.asset.as_str(),
                    })),
                    "prefund_first_trade": custody.prefund_first_trade,
                })),
            });

            Ok(LegBoundaryRequoteAttemptShape {
                outcome: LegBoundaryRequoteAttemptOutcome::Requoted {
                    order_id: input.order_id,
                    source_attempt_id: input.source_attempt_id,
                    completed_step_id: input.completed_step_id,
                    completed_leg_id: completed_leg.id,
                    plan: ExecutionPlan {
                        path_id: plan.path_id,
                        transition_decl_ids: plan.transition_decl_ids,
    ...
    #[activity]
    pub async fn materialize_leg_boundary_requote_attempt(
        self: Arc<Self>,
        _ctx: ActivityContext,
        input: MaterializeLegBoundaryRequoteInput,
    ) -> Result<LegBoundaryRequoteMaterialized, ActivityError> {
        record_activity("materialize_leg_boundary_requote_attempt", async move {
            let deps = self.deps()?;
            let LegBoundaryRequoteAttemptOutcome::Requoted {
                order_id,
                source_attempt_id,
                completed_step_id,
                completed_leg_id,
                ...
                reason,
                input_custody_snapshot,
            } = input.requote_attempt.outcome
            else {
                return Err(refresh_materialization_error(
                    "non-requoted leg boundary attempt must not be materialized",
                ));
            };
            if order_id != input.order_id.inner() {
                return Err(invariant_error(
                    "boundary_requote_attempt_order",
                    format!(
                        "boundary requote order {} does not match materialize input order {}",
                        order_id,
                        input.order_id.inner()
                    ),
                ));
            ...
            let record = deps
                .db
                .orders()
                .create_boundary_requote_execution_attempt(
                    order_id.inner(),
                    source_attempt_id.inner(),
                    completed_step_id.inner(),
                    completed_leg_id,
                    BoundaryRequoteExecutionAttemptPlan {
                        legs: plan.legs,
                        steps: plan.steps,
                        reason,
                        input_custody_snapshot,
            ...
            tracing::info!(
                ...
                event_name = "order.execution_boundary_requoted",
                "order.execution_boundary_requoted"
            );
            Ok(LegBoundaryRequoteMaterialized {
                attempt_id: record.attempt.id.into(),
                steps: record
                    .steps
                    .into_iter()
...
mod tests {
    ...
    fn step_with_request(request: Value) -> OrderExecutionStep {
        let now = Utc::now();
        OrderExecutionStep {
            id: Uuid::now_v7(),
            ...
            transition_decl_id: Some("test-transition".to_string()),
            step_index: 0,
    ...
    #[test]
    fn boundary_hyperliquid_custody_preserves_completed_step_binding() {
        let step = step_with_request(json!({
            "hyperliquid_custody_vault_role": "destination_execution",
            "hyperliquid_custody_vault_chain_id": "evm:42161",
            "hyperliquid_custody_vault_asset_id": "0xaf88d065e77c8cc2239327c5edb3a432268e5831",
            "hyperliquid_custody_vault_address": "0x835155eec87640fb4357bef00a315a04bff2f42e",
        }));

        let (role, asset, address) = boundary_step_hyperliquid_custody(&[step]);

        assert_eq!(role, CustodyVaultRole::DestinationExecution);
        assert_eq!(
            asset,
            Some(DepositAsset {
        ...
        assert_eq!(
            address.as_deref(),
            Some("0x835155eec87640fb4357bef00a315a04bff2f42e")
        );
    }

    #[test]
    fn boundary_hyperliquid_custody_defaults_to_spot_binding() {
        let step = step_with_request(json!({}));

        let (role, asset, address) = boundary_step_hyperliquid_custody(&[step]);

        assert_eq!(role, CustodyVaultRole::HyperliquidSpot);
