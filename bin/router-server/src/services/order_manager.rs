...
impl OrderManager {
    ...
    async fn best_provider_quote(
        ...
        match best {
            Some(quote) => {
                // Economic-viability gate (carried over from the multi-hop
                // overhaul, applied here to the final cross-family winner):
                // reject a quote whose estimated output is below the
                // destination asset's dust floor — too small to clear route
                // fees. Assets with no defined floor are not gated.
                if let Some(output_floor) =
                    route_output_floor(&self.asset_registry, &request.destination_asset)
                {
                    let estimated_out =
                        parse_amount("estimated_amount_out", &quote.estimated_amount_out)?;
    ...
    async fn best_multi_hop_provider_quote(
        ...
        let chosen = match outcome.chosen {
            ...
            None => {
                return Err(outcome
                    .last_error
                    .unwrap_or_else(|| MarketOrderError::NoRoute {
