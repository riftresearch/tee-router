...
WHERE operation_type IN ('across_bridge', 'hyperliquid_bridge_deposit', 'unit_deposit', 'unit_withdrawal', 'hyperliquid_trade')
...
FROM public.order_provider_operations
WHERE id = $1::uuid
  AND operation_type IN ('across_bridge', 'hyperliquid_bridge_deposit', 'unit_deposit', 'unit_withdrawal', 'hyperliquid_trade')
  AND status IN ('submitted', 'waiting_external')
"#;
